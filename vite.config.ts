...
async function copyStaticResources(targetDir: string) {
    for (const folder of ['assets', 'configs']) {
        const source = join(__dirname, 'src', folder);
        if (!existsSync(source)) continue;

        const destination = join(targetDir, folder);
        ...
        // },
        {
            ...
            configureServer(server) {
                server.httpServer!.once('listening', async () => {
                    ...
                    await copyStaticResources(pluginsDir);
                });
            },
            transform: (code, id) => {
        ...
        },
        {
            ...
            async closeBundle() {
                ...
                try {
                    ...
                    archive.directory(join(outDir, 'configs'), "configs");
